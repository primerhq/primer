...
// ============================================================================
// Custom widget: model-profile picker
// ============================================================================
//
// value is a bare ModelProfile id. A profile already pins the provider, the
// wire model name and the API-level config, so an agent's model override is
// one choice rather than a provider+model pair. Options show the resolved
// provider/model because several profiles may share one model -- the id
// alone does not say which.

function HF_ModelProfilePicker({ value, onChange, label }) {
  const { useResource, apiFetch } = window.primerApi;
  const res = useResource(
  ...
  // Keep a saved id that is no longer present visible so it is not silently
  // dropped when editing an existing binding.
  const missing = selected && !profiles.some((pr) => pr.id === selected);
  ...
  return (
    ...
    >
      <option value="">Select {label}</option>
      {missing && <option value={selected}>{selected} (missing)</option>}
      {profiles.map((pr) => (
...
function JsonSchemaForm({ schema, value, onChange, errors, _path }) {
  ...
  if (widget === "model-profile-picker") {
    const err = (errors || []).find((e) => e.path === path);
    return renderInner(
      <div className="field">
        {schema.title && <label className="field-label">{schema.title}</label>}
        <HF_ModelProfilePicker value={value} onChange={onChange} label={schema.title || "model profile"} />
        {err && <div className="field-help" style={{ color: "var(--red)" }}>{err.message}</div>}
        {schema.description && <div className="field-help">{schema.description}</div>}
      </div>
    );
