...
- **Mid-graph checkpoint/resume for human-in-the-loop.** When a node reaches a human-interaction point, the executor checkpoints its full mid-flight state, parks the session WAITING, and resumes from the checkpoint when the human answers (`snapshot_state` / `restore_state` / `resume_from_checkpoint`). Three triggers park a graph: a `_ToolCallNode` that trips a tool-approval gate, a `_ToolCallNode` whose tool is the value-yielding `system__ask_user`, and an `_AgentNodeRef` whose turn raises `YieldToWorker` from an `ask_user` / approval gate. Multiple nodes in one superstep can park together (multi-event park): the executor accumulates `_pending_toolcalls` and `_pending_agent_yields`, parks on all of them, and re-parks (`_build_pending_park_yield`) until every pending human-interaction node has been answered, so concurrent branches that each need a human do not serialise.
...
A graph run begins when a `WorkspaceSession` is created with a `GraphSessionBinding` and a `graph_input` value (validated against the Begin node's `input_schema` at session-create time and folded onto `session.metadata['graph_input']`; see `docs/dev/subsystems/sessions.md`). The worker pool builds a `WorkspaceGraphExecutor` and calls `invoke`. The Begin node materialises a seed `NodeOutput`; each superstep then runs ready nodes concurrently, streams events, applies results, and recomputes the frontier. When a node reaches a human-interaction point (a tool-approval gate, a value-yielding `ask_user` ToolCall, or an agent node that calls `ask_user`) the run suspends; an operator decision wakes it.
...
The checkpoint contract: when a node hits a human-interaction point, `_stream_node` records a `_PendingToolCall` (for a ToolCall) or a `_PendingAgentYield` (for an agent node's `ask_user` / approval gate) and posts a suspended `_NodeDone`; after the superstep settles, the loop saves state WAITING, calls `snapshot_state()` (serialising context, ready set, node states, all fan-out bookkeeping, and both pending sets), attaches it onto the `YieldToWorker` as `graph_checkpoint`, and re-raises. The worker persists it onto `ParkedState.graph_checkpoint`. `resume_from_checkpoint` restores state and, given a `resumed_tcid`, resumes only the matching entry: a pending ToolCall re-dispatches with `bypass_approval=True` for an approval gate, or feeds the operator reply back through the tool's resume hook for a value-yielding `ask_user` (`_is_value_yield_toolcall`, no re-dispatch); a pending agent-node yield resumes via `_resume_agent_node` with the operator's `agent_tool_result`. If other human-interaction nodes remain pending afterwards, the executor re-raises a `YieldToWorker` so the worker re-parks on the remaining keys (drain-until-empty). A rejected/timed-out/cancelled decision raises `_ToolApprovalRejected`, which the resume drain converts into `ended_detail='tool_execution_failed'`. The REST resume surface for these parks (`ask_user/pending`, `ask_user/respond`, `yields/{tcid}/cancel`) consults the `graph_checkpoint`'s `pending_dispatch` / `pending_agent_yields` / `pending_toolcalls` entries (`primer/api/routers/yields.py`); the broader worker/park integration is documented in `docs/dev/architecture/worker-system.md`.
...
- **Graph + yield interaction shipped narrowly as an approval-only park, then broadened to full human-in-the-loop.** Why: the executor's per-node state commit and superstep loop first needed a resume entry point, so the initial code shipped `ParkedState.graph_checkpoint` plus a resume path covering only the tool-approval gate. It was since extended to the rest of the human-interaction surface: value-yielding `ask_user` ToolCall nodes (feed the operator reply back through the tool's resume hook instead of re-dispatching) and agent-node `ask_user` / approval yields, with multi-event park (`_pending_toolcalls` + `_pending_agent_yields`, re-park-until-drained) for concurrent branches, all answerable over the channel relay and the `ask_user/pending` + `ask_user/respond` REST endpoints. Spec: docs/superpowers/specs/2026-05-22-yielding-tools-design.md.
