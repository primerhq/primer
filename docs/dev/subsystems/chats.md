...
Two design moves define the subsystem. First, **turn detachment**: a `user_message` frame arriving over the WebSocket is persisted to storage and the chat is flipped `claimable`; the worker pool's `ClaimEngine` claims it and runs the LLM turn, so the WebSocket connection's lifetime no longer gates execution. Reconnects, multi-process API hosts, and cancellations all work because storage is the source of truth and the bus is advisory. Second, **conversational soft-yield**: when a chat agent calls `ask_user` or hits an `_approval` gate, the chat does NOT park - the turn ends conversationally, the pending call is recorded on `Chat.pending_tool_call`, and the human's next `user_message` is consumed as that call's `tool_result`. (Long-park yielding tools - `sleep`, `watch_files`, `subscribe_to_trigger`, MCP tasks - belong to the session/graph surface, not chats.)

The subsystem lives in `primer/chat/` (runner, dispatch, enqueue, tick router, usage cache), `primer/model/chats.py` (the two storage models), `primer/claim/adapters/chats.py` (the claim adapter), and `primer/api/routers/chats.py` (the REST plus WebSocket surface). The worker side lives in `primer/worker/pool.py`. The yielding-tool runtime, event bus, and background tasks are shared infrastructure documented under `docs/dev/architecture/worker-system.md`; channel fan-out of `ask_user` and `_approval` gates is documented under `docs/dev/subsystems/channels.md`.

## 2. Conceptual model

A `Chat` is a top-level entity (addressable directly by the WebSocket, not nested under a workspace) that pins one `Agent` at creation. Every turn appends rows to a per-chat `ChatMessage` log keyed by a monotonic `seq`; the highest seq is mirrored onto `Chat.last_seq` as the cursor for replay. A turn is one `user_message` driven to a terminal row (`done` / `error` / `cancelled`); the FIFO queue of pending turns is implicit in the log rather than a separate table. When the agent calls `ask_user` or hits an `_approval` gate, the chat soft-yields: it records `pending_tool_call` and ends the turn (it does NOT park or share the session `parked_*` columns).
...
erDiagram
    ...
    Chat ||--o| PendingToolCall : "soft-yield gate"
...
`Agent` (`primer/model/agent.py`) is pinned via `Chat.agent_id` but is switchable mid-chat via `POST /v1/chats/{id}/agent` - a queued switch is held in `Chat.pending_handoff` and applied at the next turn, and because the agent + its system prompt are resolved fresh each turn (never stored in history) the full conversation carries over as shared context. A soft-yield records the pending call in `Chat.pending_tool_call` rather than a session-style park blob. `Subscription` (`primer/model/trigger.py`) can drive `user_message` turns into an existing chat through the canonical enqueue plus claim path.
...
- **Conversational soft-yield, not parking.** The chat row does NOT carry the session `parked_*` columns; an `ask_user` or `_approval` gate records `pending_tool_call` and ends the turn. The shared bus listener, `TimerScheduler`, `TimeoutSweeper`, watcher, and `McpTaskBridge` park-resume machinery therefore wakes parked sessions only - never chats.
...
`Chat` (`primer/model/chats.py`) carries `agent_id`, `created_at`, `status` (`active` / `ended`), `title` (derived from the first `user_message`, stamped once), `last_seq`, `turn_status` (`idle` / `claimable` / `running`), `cancel_requested_at`, `pending_tool_call` (the soft-yield gate - the call awaiting the human's next reply), and `pending_handoff` (a queued agent switch). It does NOT carry the session `parked_*` columns (chats never park), nor `claimed_by` / `claimed_at` / `last_heartbeat_at`; lease ownership lives on the `ClaimEngine` leases row, not on the chat.
...
A chat has no `parked_status` axis: a soft-yield simply ends the turn (the runner releases the lease and `on_release` writes `turn_status='idle'`) after recording `pending_tool_call`. The claim predicate gates on `turn_status` alone.

```mermaid
stateDiagram-v2
    [*] --> idle
    idle --> claimable: user_message persisted / interrupt
    claimable --> running: ClaimEngine claims (turn_status='running')
    running --> idle: queue drained (success AND drop_lease) / soft-yield (ask_user / _approval)
    running --> claimable: error / more work pending
    idle --> [*]: status='ended' (DELETE force)
```

`ChatClaimAdapter.eligibility_sql()` selects rows where `status='active' AND turn_status IN ('claimable','running')` (a `running` chat is reclaimable only after its lease TTL lapses; `resumable` was an old `parked_status` value, never a `turn_status`), and `on_release` writes `turn_status='idle'` on `(success AND drop_lease)` else `claimable`.
...
On a soft-yield the runner raises `YieldToWorker`; `run_one_chat_turn` records `pending_tool_call`, releases the chat to `turn_status='idle'`, and returns. The human's next `user_message` flips the chat to `claimable`, the next `claim_due` re-claims it, and that message is consumed as the pending call's `tool_result`. On cancel (an `interrupt` frame sets `cancel_requested_at` and publishes `chat:{id}:cancel`) the runner persists a `cancelled` row mid-stream and exits cleanly; cancel is per-turn, so a queued `user_message` proceeds normally after the dispatch loop clears `cancel_requested_at`. On reconnect the WebSocket replays rows with `seq > cursor` via `_replay_since_cursor` before subscribing to live ticks.
...
- **Chats originally reused the M1 yielding-tool park protocol, but the conversational-yield work removed chat parking entirely** (see the next decision): a chat now soft-yields via `pending_tool_call` and only sessions park. Spec: docs/superpowers/specs/2026-05-22-yielding-tools-design.md, superseded for chats by docs/superpowers/plans/2026-06-09-chat-conversational-yield.md.
