...
- **Implicit FIFO queue in the message log.** `_find_next_user_message` (`primer/chat/dispatch.py`) pages `ChatMessage` rows in ascending seq, counts terminal rows (`done` / `error` / `cancelled` / `yielded`), and returns the `(terminal_count + 1)`th `user_message`. There is no `pending_user_messages` table.
...
- Client-to-server frames: `ping`/`pong`, `user_message` (two payload shapes, `{content}` legacy and `{parts}` structured/multimodal), `interrupt` (sets `cancel_requested_at`, publishes cancel, auto-rejects any pending `_approval` park), and `tool_approval_decide` (validates `parked_state.yielded.event_key`, publishes the operator decision).
...
Tool-call approval on chats reuses the same gate as sessions: a `required` verdict parks the chat on `tool_approval:<chat>:<call_id>`. Decisions prefer the open WebSocket `tool_approval_decide` frame and fall back to `POST .../tool_approval/respond` when the socket is closed; `_maybe_auto_reject_pending_approval` auto-rejects a pending approval when a new `user_message` or `interrupt` arrives.
...
- **Tool-call approval on chats reuses the yield protocol with `tool_name='_approval'` and prefers the WebSocket `tool_approval_decide` frame, falling back to `POST respond`.** Why: the same backend park-resume path is reached either way, and the WebSocket path avoids an extra round-trip when the operator already has a live connection. Spec: docs/superpowers/specs/2026-05-24-tool-approval-system-design.md.
