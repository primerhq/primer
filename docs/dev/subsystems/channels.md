...
| `primer/channel/correlation.py` | `CorrelationStore`: `upsert_session` (atomic `INSERT ... ON CONFLICT` on the `(channel_id, anchor)` unique index), `lookup`, `delete`. |
| `primer/channel/inbound_router.py` | `ChannelInboundRouter.route_event`: resolves an anchor to its mapped session, or routes a fresh event to the channel triggers. |
