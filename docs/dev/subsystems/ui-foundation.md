...
The markdown layer: the directive dispatch in `ui/vendor/markdown.jsx` (`window.MarkdownDirectives.register`) and the embed registry under `ui/components/docs/` (`embed-registry.jsx`, `embed-preview.jsx`). The in-app docs viewer and the `/v1/user_docs/*` API that fed it were removed; the operator documentation lives in the `primerhq.github.io` Pages site, and the embed registry is what its capture harness drives.
...
The markdown directives and the embed registry are covered from the website side: `primerhq.github.io/tests/` lints the doc corpus and asserts each directive renders.
...
- **The markdown renderer dispatches custom fenced-block directives instead of JSX-only docs.** Why: a non-engineer can write a `.md` file and have it render; unknown info-strings fall through to plain code blocks so the parser stays backwards-compatible. Spec: docs/superpowers/specs/2026-06-04-user-documentation-system-design.md.
- **The in-app docs viewer and its `/v1/user_docs/*` API were removed.** Why: the operator documentation is a published site with its own build and its own lint, and serving a second copy from inside the product meant two corpora to keep in step. The markdown layer stayed, because the site's embed capture drives it.
