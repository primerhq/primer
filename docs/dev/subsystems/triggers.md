...
`Trigger.config` is a discriminated union over `kind`: `DelayedTriggerConfig` (`{kind: 'delayed', fire_at}`), `ScheduledTriggerConfig` (`{kind: 'scheduled', cron, timezone, catchup}`), `WebhookTriggerConfig` (`{kind: 'webhook', token, hmac_secret?}`), and `ChannelTriggerConfig` (`{kind: 'channel', provider_id, channel_id?}`). `Subscription.config` is a discriminated union over five kinds: `ChatMessageSubConfig`, `AgentFreshSubConfig`, `GraphFreshSubConfig`, `ParkedSessionSubConfig`, and `StartChatSubConfig`. The `parked_session` config is special: it is the one the `subscribe_to_trigger` yielding tool writes, and it points back at the parked session by `(session_id, tool_call_id, parked_at)`.
