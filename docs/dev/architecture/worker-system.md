...
`ClaimEngine` (`primer/int/claim.py`) is the lease surface. `claim_due(worker_id, max_count)` returns a batch of `Lease` records across all kinds; `heartbeat(worker_id, kind_ids)` bulk-confirms ownership and returns the still-owned subset (anything dropped is a lost lease); `release(lease, outcome)` applies a `ReleaseOutcome` (`success`, optional `requeue_after`, `last_error`, `drop_lease`); `upsert` and `mark_resumable` arm or re-arm a claim; `watch_ready` yields `(ClaimKind, entity_id)` pairs that just became claimable; `delete_lease` removes a lease (used by force-delete). `ClaimKind` enumerates `SESSION`, `HARNESS`, `TRIGGER`. The per-kind `ClaimAdapter` (eligibility SQL + `on_release` hook) is the extension point; its contract is documented in `docs/dev/architecture/claim-machine.md`.
