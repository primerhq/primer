...
**Route surface.** About 29 router modules ship under `primer/api/routers/`: `health`, `workers`, `auth`, `api_tokens`, `providers` (llm / embedding / cross_encoder / toolset / builtin_toolsets), `tools`, `semantic_search`, `web_search`, `compute` (agent / graph), `knowledge` (collection / document), `internal_collections`, `workspaces` (provider / template / workspace / sessions / files / log), `sessions`, `chats`, `yields`, `tool_approval`, `channels`, `harness`, `triggers`, `mcp_exposure`, `user_docs`, `bugs`, and the env-gated `_test_endpoints`, plus the shared helpers `_crud`, `_managed`, `_references`, `_cdc_hooks`. Notable non-CRUD shapes: the session WebSocket at `/v1/workspaces/{wid}/sessions/{sid}/ws?cursor=N`, the chat WebSocket at `/v1/chats/{id}/ws?cursor=N`, the per-session and per-chat turn-log routes (`/v1/sessions/{id}/turn_log`, `/v1/graphs/{gid}/runs/{rid}/turn_log`), the ask_user / tool-approval pending+respond endpoints, the harness `build`/`push` and trigger `fire_now` operation endpoints, and the inbound MCP mount at `/v1/mcp`. A handful of UI-driven endpoints were deferred with recorded shapes (section 8): `GET /v1/health/recent_errors`, out-of-session tool invoke, multipart file upload, and SSE session/worker streams.
...
- **CRUD-factory routers**: providers (llm / embedding / cross_encoder / toolset), `compute` (agent / graph with `managed_by_field` + `cdc_kind`), `knowledge` (collection / document), `semantic_search`, `web_search`, `channels` (provider / channel / association, the last via the flat `/v1/workspace_channel_associations` plus the scoped `/v1/workspaces/{wid}/channel_associations`), `tool_approval` policies, `api_tokens`, and `mcp_exposure`. These declare reference checks and managed-by guards through the factory parameters rather than bespoke handlers.
