...
- `ChatClaimAdapter` (`primer/claim/adapters/chats.py`): `eligibility_sql()` reads JSONB, `e.data->>'status' = 'active' AND e.data->>'turn_status' IN ('claimable','running')` (a `running` chat is reclaimable only once its lease TTL lapses). Chats never park, so there is no `parked_status` term; `resumable` was an old `parked_status` value, never a `turn_status`. `on_release` flips `turn_status` to `idle` on `(success AND drop_lease)` and to `claimable` otherwise. It does not clear `cancel_requested_at` (a known gap; see section 8).
