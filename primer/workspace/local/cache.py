"""Per-session output caching for the workspace's ``.tmp/`` directory (local-FS).
...
class TruncatedOutput(BaseModel):
    """The result of running text through :meth:`LocalTruncationStore.output`."""
...
class LocalTruncationStore:
    """Per-workspace store for oversized tool outputs, host-FS backed.
    ...
    async def write(self, text: str, *, session_id: str) -> Path:
        ...
        logger.debug(
            "LocalTruncationStore wrote cache file",
            extra={
    ...
    async def cleanup(self) -> int:
        ...
        if removed:
            logger.info(
                "LocalTruncationStore retention sweep removed cache files",
    ...
    async def cleanup_session(self, session_id: str) -> int:
        ...
        logger.info(
            "LocalTruncationStore reaped session cache directory",
    ...
    async def _background_loop(self, interval_seconds: float) -> None:
        while True:
            ...
            try:
                ...
                logger.warning(
                    "LocalTruncationStore background cleanup failed",
