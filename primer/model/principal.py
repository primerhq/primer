...
class Principal(BaseModel):
    ...
    (``"local"`` cookie, ``"internal"`` bearer token, ``"system"`` when
    auth is disabled). ``role`` is the RBAC role the request runs as, or
    ``None`` when unknown.
    """
    ...
    source: str = Field(
        ..., description="Auth channel: local | internal | system.",
    )
