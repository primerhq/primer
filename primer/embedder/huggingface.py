...
# Model-family → prompt mapping is shared with the OpenAI embedder so
# both adapters apply the same prefix discipline when serving an
# asymmetric-retrieval model (BGE, E5, nomic-embed-text). See
# ``primer.embedder._prompts`` for the registry.
from primer.embedder._prompts import (
    resolve_prompts_for_model as _resolve_prompts_for_model,
    select_prompt as _select_prompt_shared,
)
...
def _select_prompt(
    *, task_type: str | None, model_name: str, raw: dict
    ...
    """Pick the prompt prefix for this call (delegates to the shared module).

    Signature kept stable so existing tests that import this symbol from
    ``primer.embedder.huggingface`` keep working without modification.
    """
    return _select_prompt_shared(
        task_type=task_type, model_name=model_name, raw=raw,
    )
