...
@pytest.mark.asyncio
async def test_t0510_post_entity_with_empty_string_id_autogenerates(
    client: httpx.AsyncClient,
) -> None:
    """T0510 — Identifiable.id is optional: an empty (or omitted) id
    autogenerates ``<prefix>-<hex>`` rather than persisting an empty,
    unaddressable id. POST with id="" must therefore return 201 with a
    real, addressable id (never /errors/internal, never an empty id).
    """
    import re as _re
    ...
    assert resp.status_code == 201, (
        f"empty id should autogenerate (201); got "
        f"{resp.status_code}: {resp.text}"
    )
    new_id = envelope["id"]
    assert _re.fullmatch(r"toolset-[0-9a-f]{12}", new_id), new_id
    # The autogenerated id is addressable.
    got = await client.get(f"/v1/toolsets/{new_id}")
    assert got.status_code == 200, got.text
    await client.delete(f"/v1/toolsets/{new_id}")
