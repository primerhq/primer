...
from matrix.api.config import ObservabilityConfig
import matrix.observability.tracing as tracing_module
from matrix.observability.tracing import get_tracer, setup
...
class TestGetTracer:
    ...
    def test_get_tracer_after_setup_returns_tracer(self) -> None:
        ...
        cfg = ObservabilityConfig()
        setup(cfg)
        t = get_tracer("matrix.observability.test")
        assert isinstance(t, otel_trace.Tracer)
